  ...
  return (
    <article className="detail-page" style={{ '--accent': project.accent } as React.CSSProperties}>
      ...
      <div className="container detail-layout">
        <aside className="detail-nav"><span>CONTENTS</span>{['overview','background','role','features','stack','trouble','screens','docs','retrospect'].map((id, i) => <a href={`#${id}`} key={id}>{String(i + 1).padStart(2, '0')}</a>)}</aside>
        <div className="detail-content">
          ...
          <section id="trouble" className="detail-section"><span className="section-number">06 / PROBLEM SOLVING</span><h2>문제 해결 경험</h2>{project.troubleshooting.map((item) => <div className="trouble-card" key={item.title}><h3>{item.title}</h3><dl><div><dt>Situation</dt><dd>{item.situation}</dd></div><div><dt>Solution</dt><dd>{item.solution}</dd></div><div><dt>Result</dt><dd>{item.result}</dd></div></dl></div>)}</section>
          <section id="screens" className="detail-section"><span className="section-number">07 / SCREENS</span><h2>화면 캡처</h2><div className="screenshots">{project.screenshots.map((image) => <figure key={image.src}><img src={`${import.meta.env.BASE_URL}${image.src.replace(/^\//, '')}`} alt={image.alt} /><figcaption>{image.caption}</figcaption></figure>)}</div></section>
          <section id="docs" className="detail-section"><span className="section-number">08 / RESOURCES</span><h2>문서와 링크</h2><div className="resource-grid">{project.documents.map((doc) => <ResourceLink key={doc.label} {...doc} />)}<ResourceLink {...project.deploy} /><ResourceLink {...project.github} /></div></section>
          <section id="retrospect" className="detail-section"><span className="section-number">09 / RETROSPECTIVE</span><h2>회고</h2><blockquote>{project.retrospective}</blockquote></section>
