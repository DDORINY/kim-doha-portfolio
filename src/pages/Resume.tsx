  ...
  return (
    <section className="section page-section"><div className="container resume-page">
      ...
      <div className="resume-grid">
        ...
        <section className="resume-block full"><span className="section-number">06 / INFRASTRUCTURE</span><h2>인프라 구축 및 배포 환경</h2><p>{profile.infra.summary}</p><ul className="check-list">{profile.infra.highlights.map((item) => <li key={item}>{item}</li>)}</ul></section>
        <section className="resume-block full"><span className="section-number">07 / WORK EXPERIENCE</span><h2>경력 사항</h2><div className="work-list">{profile.workExperience.map((job) => (
        ...
        <section className="resume-block full"><span className="section-number">08 / PROJECTS</span><h2>프로젝트 요약</h2><div className="resume-projects">{projects.map((project) => <Link to={`/projects/${project.slug}`} key={project.slug}><div><span>{project.type}</span><h3>{project.name}</h3><p>{project.tagline}</p></div><strong>↗</strong></Link>)}</div></section>
      </div>
    </div></section>
  )
}
