...
function PrintButton({ className = 'button primary' }: { className?: string }) {
  return <button className={className} onClick={() => window.print()} title="브라우저 인쇄 대화상자에서 PDF로 저장할 수 있습니다">SAVE RESUME AS PDF</button>
}
    ...
    <div className="resume-page">
      ...
      <div className="resume-print-page">
        <div className="print-page print-page-1">
          ...
          <section className="print-section"><h2>대표 프로젝트</h2>{projects.slice(0, 2).map((project) => <PrintProject project={project} key={project.slug} />)}</section>
        </div>
        <div className="print-page print-page-2">
          <section className="print-section"><h2>대표 프로젝트 (계속)</h2>{projects.slice(2, 3).map((project) => <PrintProject project={project} key={project.slug} />)}</section>
