  ...
  return (
    <section className="section page-section"><div className="container resume-page">
      ...
      <div className="resume-grid">
        ...
        <section className="resume-block full"><span className="section-number">07 / PROJECTS</span><h2>프로젝트 요약</h2><div className="resume-projects">{projects.map((project) => <Link to={`/projects/${project.slug}`} key={project.slug}><div><span>{project.type}</span><h3>{project.name}</h3><p>{project.tagline}</p></div><strong>↗</strong></Link>)}</div></section>
      </div>
    </div></section>
  )
}
