  ...
  return (
    <div className="print-project">
      ...
      {project.resumeHighlight && (
        <p className="print-project-role"><b>역할</b> {project.resumeHighlight.role} · <b>범위</b> {project.resumeHighlight.contribution}</p>
      )}
...
function PrintButton({ className = 'button primary' }: { className?: string }) {
  return <button type="button" className={className} onClick={() => window.print()} title="브라우저 인쇄 대화상자에서 PDF로 저장할 수 있습니다" aria-label="브라우저 인쇄 대화상자를 열어 이력서를 PDF로 저장">SAVE RESUME AS PDF</button>
}
    ...
    <div className="resume-page">
      <div className="resume-screen-only resume-web">
        <section className="resume-web-hero page-section" aria-labelledby="resume-page-title">
          <div className="container resume-web-hero-grid">
            <Reveal className="resume-web-hero-copy">
              ...
              <p className="resume-web-specialty">AI Backend · Computer Vision · Service Integration</p>
              <h2>컴퓨터 비전 모델과 Python 백엔드를<br />실제 웹 서비스로 연결합니다.</h2>
              ...
              <div className="resume-web-tech" aria-label="핵심 기술">
                {['Python', 'Flask / FastAPI', 'YOLO', 'MySQL', 'Docker / Linux'].map((tech) => <span key={tech}>{tech}</span>)}
              </div>
            </Reveal>
            <Reveal className="resume-web-profile" delay={80}>
              ...
              <div className="resume-web-contact">
                ...
                <a href={profile.github} target="_blank" rel="noreferrer" aria-label="GITHUB github.com/DDORINY 새 창에서 열기">GITHUB <small>github.com/DDORINY ↗</small></a>
                <PrintButton />
              </div>
            </Reveal>
          </div>
        ...
        <section className="section resume-summary-section" aria-labelledby="resume-summary-title">
          <div className="container">
            ...
            <Reveal className="resume-summary-copy">{resumeProfileSummary.map((summary) => <p key={summary}>{summary}</p>)}</Reveal>
          </div>
        </section>
        ...
        <section className="section" aria-labelledby="resume-competencies-title">
          <div className="container">
            ...
            <div className="resume-competency-grid">{resumeCompetencies.map((competency, index) => <Reveal as="article" className="resume-competency-card" delay={index * 55} key={competency.id}><div className="resume-competency-head"><span>{competency.code}</span><TechIcon name={competency.icon} /></div><h3>{competency.title}</h3><p>{competency.description}</p><div className="chip-row">{competency.tech.map((tech) => <TechChip label={tech} key={tech} />)}</div></Reveal>)}</div>
          </div>
        </section>

        <section className="section resume-project-section" aria-labelledby="resume-projects-title">
          <div className="container">
            <SectionHeading id="resume-projects-title" eyebrow="04 / REPRESENTATIVE PROJECTS" title="REPRESENTATIVE PROJECTS" description="AI 서비스 통합, 백엔드·DB 리딩, 개인 Full-stack 배포 경험을 비교할 수 있는 3개 프로젝트입니다." />
            <div className="resume-project-list">{representativeResumeProjects.map((project, index) => <Reveal as="article" className="resume-project-row" delay={index * 60} key={project.slug}>
              <div className="resume-project-index"><span>0{index + 1}</span><b className={project.status === 'in-progress' ? 'is-progress' : ''}>{project.status === 'in-progress' ? 'IN PROGRESS' : 'COMPLETED'}</b></div>
              <div className="resume-project-main"><span>{project.period} · {project.teamNote}</span><h3>{project.name}</h3><p>{project.tagline}</p></div>
              <dl className="resume-project-evidence"><div><dt>ROLE</dt><dd>{project.resumeHighlight?.role}</dd></div><div><dt>{project.teamNote?.startsWith('개인') ? 'SCOPE' : 'CONTRIBUTION'}</dt><dd>{project.resumeHighlight?.contribution}</dd></div><div><dt>EVIDENCE</dt><dd>{project.resumeHighlight?.achievement}</dd></div><div><dt>STACK</dt><dd><span className="chip-row">{(project.techHighlights ?? []).slice(0, 4).map((tech) => <TechChip label={tech} key={tech} />)}</span></dd></div></dl>
              <Link className="resume-project-link" to={`/projects/${project.slug}`}>VIEW PROJECT DETAIL <span>→</span></Link>
            </Reveal>)}</div>
          </div>
        </section>
        ...
        <section className="section resume-skills-section" aria-labelledby="resume-skills-title">
          <div className="container">
            ...
            <div className="resume-skill-grid">{resumeSkillGroups.map((group, index) => <Reveal as="article" className="resume-skill-group" delay={index * 55} key={group.label}><span>{group.label}</span><h3>{group.description}</h3><div className="chip-row">{group.skills.map((skill) => <TechChip label={skill} key={skill} />)}</div></Reveal>)}</div>
          </div>
        </section>
        ...
        <section className="section resume-service-section" aria-labelledby="resume-service-title">
          <div className="container">
            ...
            <div className="resume-service-grid">{serviceExperience.map((item, index) => <Reveal as="article" className="resume-service-step" delay={index * 50} key={item.code}>
              <div><span>{item.code}</span><b>{item.scope}</b></div>
              <h3>{item.title}</h3>
              <div className="resume-service-tags">{item.environment.map((tech) => <span key={tech}>{tech}</span>)}</div>
      ...
      <div className="resume-print-page">
        <div className="print-page print-page-1">
          <div className="print-hero"><div><h1>{profile.name}</h1><p className="print-role">{profile.positioning.position}</p><p className="print-contact">{profile.email} · github.com/DDORINY · ddoriny.com</p></div><img src={profile.photo} alt={`${profile.name} 증명사진`} /></div>
          <ul className="print-summary">{profile.resumeSummary.map((line) => <li key={line}>{line}</li>)}</ul>
          <section className="print-section"><h2>핵심 역량</h2><ul className="print-list">{profile.coreCompetencies.map((item) => <li key={item}>{item}</li>)}</ul></section>
          <section className="print-section"><h2>대표 프로젝트</h2>{representativeResumeProjects.slice(0, 2).map((project) => <PrintProject project={project} key={project.slug} />)}</section>
        </div>
        <div className="print-page print-page-2">
          <section className="print-section"><h2>대표 프로젝트 (계속)</h2>{representativeResumeProjects.slice(2, 3).map((project) => <PrintProject project={project} key={project.slug} />)}</section>
          <section className="print-section"><h2>기술 스택</h2><div className="print-skills">{Object.entries(profile.skills).map(([group, skills]) => <p key={group}><b>{group}</b> {skills.join(', ')}</p>)}</div></section>
          <section className="print-section"><h2>경력 요약</h2><ul className="print-list">{profile.careerSummary.map((item) => <li key={item.label}><b>{item.label}</b> {item.value}</li>)}</ul></section>
          <section className="print-section"><h2>교육 · 자격증</h2><ul className="print-list">{profile.education.map((item) => <li key={item.title}><b>{item.title}</b> — {item.period}</li>)}{profile.certifications.map((cert) => <li key={cert.title}><b>{cert.title}</b> — {cert.issuer} · {cert.date}</li>)}</ul></section>
