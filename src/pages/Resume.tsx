  ...
  return (
    <section className="section page-section"><div className="container resume-page">
      ...
      <div className="resume-grid">
        <section className="resume-block full"><span className="section-number">01 / CORE COMPETENCIES</span><h2>핵심 역량</h2><ul className="check-list" style={{ gridTemplateColumns: '1fr', gap: 10, marginTop: 12 }}>{profile.coreCompetencies.map((item) => <li key={item}>{item}</li>)}</ul></section>
        <section className="resume-block full"><span className="section-number">02 / CAREER SUMMARY</span><h2>경력 요약</h2><dl className="summary-table">{profile.careerSummary.map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl></section>
        <section className="resume-block full"><span className="section-number">03 / ABOUT</span><h2>자기소개</h2><div className="about-text">{profile.about.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div></section>
        <section className="resume-block"><span className="section-number">04 / EDUCATION</span><h2>교육</h2>{profile.education.map((item) => <div className="timeline-item" key={item.title}><span>{item.period}</span><div><h3>{item.title}</h3><p>{item.description}</p></div></div>)}</section>
        <section className="resume-block"><span className="section-number">05 / DESIRED ROLES</span><h2>희망 직무</h2><div className="large-chip-row">{profile.desiredRoles.map((role) => <span key={role}>{role}</span>)}</div></section>
        <section className="resume-block full"><span className="section-number">06 / CERTIFICATIONS</span><h2>자격증</h2><div className="cert-list">{profile.certifications.map((cert) => <div className="cert-item" key={cert.title}><h3>{cert.title}</h3><p>{cert.issuer} · {cert.date}</p></div>)}</div></section>
        <section className="resume-block full"><span className="section-number">07 / SKILLS</span><h2>기술 스택</h2><div className="skills-grid">{Object.entries(profile.skills).map(([group, skills]) => {
        ...
        <section className="resume-block full"><span className="section-number">08 / INFRASTRUCTURE</span><h2>배포 및 인프라 경험</h2><p>{profile.infra.summary}</p><div className="cert-list">{profile.infra.categories.map((category) => (
        ...
        <section className="resume-block full"><span className="section-number">09 / WORK EXPERIENCE</span><h2>경력 사항</h2><div className="work-list">{profile.workExperience.map((job) => (
        ...
        <section className="resume-block full"><span className="section-number">10 / PROJECTS</span><h2>프로젝트 요약</h2><div className="resume-projects">{projects.map((project) => (
