...
export const projects: Project[] = [
  {
    ...
    tagline: 'YOLO11 기반 CCTV 영상 분석으로 고속도로 정차 차량과 갓길 정차 이벤트를 탐지하고, 관제자가 실시간으로 확인할 수 있도록 구축한 AI 관제 MVP 시스템',
    teamNote: '팀 프로젝트 · AI 모델 실험·서비스 연동·통합 QA 담당',
    categories: ['AI / Computer Vision', 'Infra / Deployment'],
    highlights: ['YOLO11', 'Keras 실험', 'BBOX Overlay', 'VM 분리 운영', 'Socket.IO', 'AI Media Proxy'],
    ...
    summary: '단순 웹 서비스가 아니라 Frontend VM · Flask VM · AI VM · DB VM을 분리해 운영한 AI 서비스 통합 프로젝트입니다. CCTV 영상에서 YOLO11로 차량을 탐지하고 ROI·이동량 기준으로 정차·갓길 정차 이벤트를 판단해, Flask API Gateway를 거쳐 MySQL에 저장하고 Socket.IO로 관제 화면에 실시간 전달합니다. 이벤트 목록·상세·스냅샷·MP4 replay까지 이어지는 흐름을 실제 운영 가능한 MVP로 구현했습니다.',
    ...
    role: [
      ...
      'YOLO 계열 및 RT-DETR 모델 학습',
      'Precision·Recall·F1·mAP 기반 모델 성능 비교와 최종 모델 선정 참여',
      '프론트엔드 MVP 개발',
    ...
    techStack: ['Next.js', 'TypeScript', 'React', 'Nginx', 'Flask', 'Python', 'JWT', 'Socket.IO', 'FastAPI', 'YOLO11', 'RT-DETR', 'OpenCV', 'TensorFlow', 'Keras', 'MySQL', 'SQLAlchemy', 'Linux', 'Ubuntu', 'VMware', 'systemd', 'SSH', 'Git', 'GitHub'],
    systemFlow: [
      ...
      { label: '차량 탐지', description: 'AI VM이 CCTV 프레임에서 YOLO11 기반 차량 탐지 수행' },
    ...
    extraSections: [
      ...
      { id: 'achievements', insertAfter: 'trouble', label: 'ACHIEVEMENTS', heading: '성과', items: [
        ...
        'YOLO11 기반 정차 차량 탐지와 ROI 기반 이상 이벤트 판단 구조 이해 및 통합',
    ...
    architectureFlow: [
      ...
      { label: 'AI VM / YOLO Detection', sub: 'FastAPI · YOLO11' },
    ...
    projectAreas: [
      ...
      {
        id: 'ai', label: 'AI',
        summary: 'FastAPI 기반 AI VM에서 YOLO11로 CCTV 차량을 탐지하고, ROI와 이동량 기준으로 정차·갓길 정차 이벤트를 판단해 Flask VM으로 전달합니다.',
        techStack: ['Python', 'FastAPI', 'YOLO11', 'RT-DETR', 'OpenCV', 'TensorFlow', 'Keras'],
        architecture: [
          { label: 'CCTV Ingest', description: 'RTSP/HLS 스트림 수신 및 카메라 워커' },
          { label: 'YOLO11 Detector', description: '차량 탐지 및 bbox·confidence 산출' },
        ...
        implementationPoints: [
          'YOLO11 탐지 결과를 bbox·class·confidence·track_id·source 등 표준화된 Detection 데이터로 변환',
        ...
        evidence: [
          { label: '탐지 모델', value: 'YOLO11(실시간) · RT-DETR · Keras 비교실험' },
  ...
  },
  {
    ...
    deploy: { label: '배포 사이트', url: 'https://404-rnf.ddoriny.com/' }, github: { label: 'GitHub 저장소', placeholder: true },
